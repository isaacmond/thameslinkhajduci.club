  ...
  return (
    <div className="space-y-3">
      ...
      <ul className="divide-y divide-white/10">
        {items.map((it) => (
          <li key={it.id} className="flex flex-col gap-3 py-3 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div className="flex shrink-0 gap-2">
              <button type="button" disabled={pending} onClick={() => run(it.id, approveSubmissionAction)} className="focus-ring inline-flex items-center gap-1.5 rounded-lg bg-mint px-3 py-2 text-sm font-semibold text-night hover:bg-mint-soft disabled:opacity-50"><Check size={16} aria-hidden />{busy === it.id ? "Recording…" : "Record it"}</button>
