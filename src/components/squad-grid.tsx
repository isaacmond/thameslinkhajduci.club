...
import { fmtDate, initials } from "@/lib/stats";
...
const SORTS: { key: SortKey; label: string }[] = [
  ...
  { key: "winRate", label: "Win %" }, { key: "motm", label: "MOTM" }, { key: "debut", label: "Debut" }, { key: "name", label: "Name" },
];
...
export function SquadGrid({ players, currentSeason }: { players: SquadCard[]; currentSeason: string | null }) {
  ...
  const filterLabel = (f: Filter) => f === "all" ? `All (${players.length})` : f === "active" ? `${currentSeason ?? "This season"} squad (${players.filter((p) => p.activeThisSeason).length})` : `${f} (${players.filter((p) => p.positions.includes(f)).length})`;

  return (
    <div>
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
        <div className="flex flex-wrap gap-1.5" role="group" aria-label="Filter players">
          {FILTERS.filter((f) => f !== "active" || currentSeason).map((f) => (
            <button key={f} type="button" onClick={() => setFilter(f)} aria-pressed={filter === f} className={clsx("focus-ring chip cursor-pointer transition-colors", filter === f ? "border-mint/50 bg-mint/15 text-mint-soft" : "text-ash hover:text-cream")}>{filterLabel(f)}</button>
          ))}
        </div>
        <label className="flex items-center gap-2 text-xs text-ash">
          <span className="eyebrow">Sort</span>
          <select value={sort} onChange={(e) => setSort(e.target.value as SortKey)} className="focus-ring rounded-md border border-white/10 bg-white/5 px-2 py-1.5 text-sm text-cream">
            {SORTS.map((s) => <option key={s.key} value={s.key} className="bg-pine">{s.label}</option>)}
          </select>
        </label>
      </div>
          ...
          <li key={p.slug} className="animate-rise" style={{ animationDelay: `${Math.min(i, 12) * 40}ms` }}>
            <Link href={`/squad/${p.slug}`} className="focus-ring card group block h-full overflow-hidden transition-transform hover:-translate-y-0.5">
              <ViewTransition name={`player-${p.slug}`} share="morph" default="none"><div className="relative aspect-[4/5] overflow-hidden bg-gradient-to-br from-forest to-pine">
                ...
                {p.shirt !== null && <span className="display absolute right-2 top-2 flex h-9 w-9 items-center justify-center rounded-full bg-gold text-lg text-night shadow">{p.shirt}</span>}
                ...
                <div className="absolute inset-x-0 bottom-0 p-3">
                  <h3 className="display text-2xl leading-none text-cream drop-shadow">{p.name}</h3>
