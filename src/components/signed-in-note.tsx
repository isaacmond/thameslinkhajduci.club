...
export function SignedInNote({ signedIn }: { signedIn: SignedIn }) {
  return (
    <div className="flex flex-col gap-1 text-xs text-ash">
      ...
      <p className="flex min-h-[2.4rem] items-center gap-2 text-sm text-cream"><BadgeCheck size={16} className="text-mint" aria-hidden />{signedIn.player} <span className="text-ash">· signed in</span></p>
