...
export type PlayerRow = { name: string; slug: string; apps: number; goals: number; assists: number; motm: number; champagne: number; wins: number; draws: number; losses: number; goalsPerGame: number; assistsPerGame: number; gpgGames: number; apgGames: number; winRate: number; ppg: number };
...
const COLS: { key: Key; label: string; title?: string; num?: boolean }[] = [
  ...
  { key: "goalsPerGame", label: "G/G", title: "Goals per game (games with scorers logged)", num: true }, { key: "assistsPerGame", label: "A/G", title: "Assists per game (games with assists logged)", num: true }, { key: "motm", label: "MOTM", title: "Man of the match awards", num: true }, { key: "champagne", label: "🍾", title: "Champagne moments", num: true },
  ...
  return (
    <div>
      ...
      <div className="scroll-x card overflow-x-auto">
        <table className="stats min-w-[880px]">
          <thead>
            <tr>
              <th className="w-8">#</th>
              {COLS.map((c) => (
                <th key={c.key} className={clsx(c.num && "num")} aria-sort={sort.key === c.key ? (sort.dir === -1 ? "descending" : "ascending") : undefined}>
                  <button type="button" onClick={() => click(c.key)} title={c.title} className={clsx("focus-ring inline-flex items-center gap-1 rounded uppercase tracking-[0.16em] hover:text-cream", sort.key === c.key && "text-mint-soft")}>{c.key === "champagne" ? <><span aria-hidden>🍾</span><span className="sr-only">Champagne moments</span></> : c.label}<span aria-hidden className="text-[9px]">{sort.key === c.key ? (sort.dir === -1 ? "▼" : "▲") : ""}</span></button>
                </th>
              ))}
            </tr>
          </thead>
          <tbody>
            {rows.map((r, i) => (
              <tr key={r.slug}>
                ...
                <td className="num">{r.apps}</td><td className="num font-semibold text-cream">{r.goals}</td><td className="num">{r.assists}</td><td className="num" title={`${r.goals} goals over ${r.gpgGames} games with scorers logged`}>{r.goalsPerGame.toFixed(2)}</td><td className="num" title={`${r.assists} assists over ${r.apgGames} games with assists logged`}>{r.assistsPerGame.toFixed(2)}</td>
