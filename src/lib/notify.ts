 ...
 */
export async function emailScoreSubmission(input: { subject: string; text: string; summary: string; edits: { cell: string; value: string | number; what: string }[]; tab: string | null; sheetUrl: string; submittedBy: string; kind?: "score" | "payment" | "player"; applied?: boolean }): Promise<boolean> {
  ...
  <div style="max-width:640px;margin:0 auto;padding:28px 20px">
    <p style="margin:0 0 6px;font-size:11px;letter-spacing:.22em;text-transform:uppercase;color:#a7b8ab">${label} ${input.applied ? "recorded" : "submission"} · Thameslink Hajduci</p>
    ...
    <h2 style="margin:22px 0 8px;font-size:14px;letter-spacing:.12em;text-transform:uppercase;color:#a7b8ab">${input.applied ? "Cells written" : "Cells to change"}${input.tab ? ` · tab ${esc(input.tab)}` : ""}</h2>
    ...
    <p style="margin:18px 0 0;font-size:12px;color:#a7b8ab">${input.applied ? `Recorded by ${esc(input.submittedBy)}, signed in. Already in the sheet; the site follows within a minute. This email is your copy.` : `Submitted by ${esc(input.submittedBy)}. Nothing changes on the site until you apply this.`}</p>
  </div></body></html>`;
  try {
