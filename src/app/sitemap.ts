...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const now = new Date();
  const fixed = ["", "/squad", "/matches", "/seasons", "/stats", "/records", "/money", "/data", "/submit"].map((p) => ({ url: `${SITE_URL}${p}`, lastModified: now, changeFrequency: "daily" as const, priority: p === "" ? 1 : 0.8 }));
  return [
    ...fixed,
