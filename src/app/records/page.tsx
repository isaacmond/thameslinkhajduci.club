...
function Card({ icon, label, value, sub, href, tone = "default" }: { icon: React.ReactNode; label: string; value: React.ReactNode; sub?: React.ReactNode; href?: string; tone?: "default" | "good" | "bad" | "gold" }) {
  const inner = (
    <>
      <span className={clsx(tone === "good" && "text-mint-soft", tone === "bad" && "text-[#ff9a9d]", tone === "gold" && "text-gold", tone === "default" && "text-ash")}>{icon}</span>
      <p className="eyebrow mt-3">{label}</p>
      <p className="display mt-1 text-4xl leading-none text-cream">{value}</p>
      {sub && <p className="mt-1 text-sm text-ash">{sub}</p>}
    </>
  );
  const cls = clsx("card block h-full p-5", href && "focus-ring transition-colors hover:border-white/20");
