        ...
        <section className="card overflow-hidden">
          ...
          <div className="scroll-x overflow-x-auto">
            <table className="stats w-full sm:min-w-[560px]">
              ...
              <tbody>
                {[...played, ...future.slice(0, 1)].map((m) => <tr key={m.id} className={clsx(!m.played && "opacity-70")}><td><span className="flex items-center gap-2"><Link href={`/matches/${m.id}`} className="link block max-w-[7rem] truncate font-medium text-cream sm:max-w-none" title={m.opponent}>{m.opponent}</Link>{!m.played && <span className="chip hidden text-ash sm:inline-flex">Next</span>}</span></td><td><span className="flex items-center gap-2"><ResultPill result={m.result} size="sm" />{m.played && <span className="text-xs text-ash">{scoreline(m)}</span>}</span></td><td className="num">{m.played ? m.playersInGame : "–"}</td><td className="num">{fmtMoney(m.matchCost)}</td><td className="num">{m.played && m.costPerPlayer ? fmtMoney(m.costPerPlayer) : "–"}</td><td className="hidden text-ash sm:table-cell">{fmtDate(m.date)}</td></tr>)}
              </tbody>
            </table>
          </div>
          {future.length > 1 && (
