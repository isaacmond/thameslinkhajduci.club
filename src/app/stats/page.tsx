...
const toRow = (p: Player): PlayerRow => { const d = p.wins + p.draws + p.losses; return { name: p.name, slug: p.slug, apps: p.apps, goals: p.goals, assists: p.assists, motm: p.motm, champagne: p.champagne, wins: p.wins, draws: p.draws, losses: p.losses, goalsPerGame: p.goalsPerGame, winRate: p.winRate, ppg: d ? +((p.wins * 3 + p.draws) / d).toFixed(2) : 0 }; };
...
export default async function StatsPage() {
  ...
  const boards: { title: string; sub?: string; items: { player: Player; value: number }[]; color?: string; fmt?: (v: number) => string }[] = [
    { title: "Appearances", items: leaderboard(data.players, "apps").slice(0, 5), color: "bg-blue-400" },
    { title: "Goals", items: leaderboard(data.players, "goals").slice(0, 5) },
    { title: "Assists", sub: "Self-reported", items: leaderboard(data.players, "assists").slice(0, 5), color: "bg-[#c4871a]" },
    { title: "Goal involvements", sub: "Goals + assists", items: leaderboard(data.players, "ga").slice(0, 5), color: "bg-[#c95a9b]" },
    { title: "Man of the match", items: leaderboard(data.players, "motm").slice(0, 5), color: "bg-gold" },
    { title: "Champagne moments", sub: "The wall of shame", items: leaderboard(data.players, "champagne").slice(0, 5), color: "bg-peach" },
    { title: "Goals per game", sub: `Minimum ${MIN} apps`, items: leaderboard(data.players, "goalsPerGame", MIN).slice(0, 5), fmt: (v) => v.toFixed(2) },
    { title: "Win rate", sub: `Minimum ${MIN} apps`, items: leaderboard(data.players, "winRate", MIN).slice(0, 5), fmt: (v) => `${v}%`, color: "bg-mint" },
  ];
  ...
  return (
    <div className="space-y-10">
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <div className="card p-5"><SectionTitle sub="Top 15 scorers, split by season">Goals by season</SectionTitle><StackedBySeason data={stack("goals")} seasons={seasonIds} /></div>
