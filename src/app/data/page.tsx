  ...
  return (
    <div className="space-y-10">
      ...
      <section className="grid gap-4 sm:grid-cols-3">
        <div className="card p-5"><p className="eyebrow">Source of truth</p><a href={SHEET_URL} target="_blank" rel="noopener noreferrer" className="link mt-1 inline-flex items-center gap-1 text-lg font-medium">The spreadsheet <ExternalLink size={14} aria-hidden /></a><p className="mt-1 text-xs text-ash">Season tabs S1–S{data.seasons.length}, Money, Payments</p></div>
        <div className="card p-5"><p className="eyebrow">Last read</p><p className="display mt-1 text-2xl text-cream">{readAt}</p><p className="mt-1 text-xs text-ash">Re-read at most every {REVALIDATE_SECONDS}s, or on demand with the button</p></div>
        <div className="card p-5"><p className="eyebrow">In the dataset</p><p className="display mt-1 text-2xl text-cream">{data.matches.length} fixtures · {data.players.length} players</p><p className="mt-1 text-xs text-ash">{data.matches.filter((m) => m.played).length} played, {data.allTime.goalsFor + data.allTime.goalsAgainst} goals witnessed</p></div>
      </section>
      ...
      <section>
        ...
        <DataExplorer tables={TABLES.map((t) => ({ name: t, info: TABLE_INFO[t] }))} seasons={[...data.seasons].reverse().map((s) => s.id)} siteUrl={SITE} />
      </section>
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <div className="card p-5">
          ...
          <pre className="overflow-x-auto rounded-lg bg-night/70 p-4 text-xs text-cream/90"><code>{`GET /api/export?table=<name>&format=csv|json|md[&season=S7]\n\ntables: ${TABLES.join(", ")}\n\n# examples\n${SITE}/api/export?table=players&format=csv\n${SITE}/api/export?table=goals&season=S7&format=json\n${SITE}/api/export?table=opponents&format=md`}</code></pre>
          <ul className="mt-4 space-y-1 text-sm text-ash">{TABLES.map((t) => <li key={t}><Link href={`/api/export?table=${t}&format=json`} className="link font-mono text-xs">{t}</Link> <span>· {TABLE_INFO[t]}</span></li>)}</ul>
        </div>
        <div className="space-y-6">
          <div className="card p-5">
            ...
            <pre className="overflow-x-auto rounded-lg bg-night/70 p-4 text-xs text-cream/90"><code>{`GET /api/data           # everything, nested JSON\nGET /api/data?pretty=1  # same, indented for humans\nPOST /api/revalidate    # force a fresh read of the sheet`}</code></pre>
            <p className="mt-3 text-sm text-ash">The JSON has <code className="text-cream">seasons[].matches[].lineup[]</code>, <code className="text-cream">players[].seasons[]</code> and <code className="text-cream">money</code>, all with the same rules the site uses (friendlies and forfeits flagged with <code className="text-cream">countsForRecords: false</code>). CORS is open, responses are cached for a minute.</p>
            <Link href="/api/data?pretty=1" className="link mt-3 inline-flex items-center gap-1 text-sm">Open /api/data <ExternalLink size={14} aria-hidden /></Link>
          </div>
