...
export default async function AccountPage() {
  ...
  const member = memberFor(user.email);
  if (!member) {
    return (
      <PageTransition><div className="space-y-6">
        ...
        <div className="card flex flex-wrap items-center gap-4 p-5"><UserRoundX size={20} className="text-ash" aria-hidden /><p className="text-sm text-ash">Anyone can still use the <Link href="/submit" className="link">Submit page</Link>; the admin applies those by hand.</p><span className="ml-auto"><SignOut /></span></div>
      </div></PageTransition>
    );
  }
  ...
  return (
    ...
    <div className="space-y-8">
      <PageHeader eyebrow="Your account" title={member.player} sub={<>Your details as the site shows them. Submissions you make while signed in go straight into the records{sheetsConfigured() ? "" : " once the admin finishes the Google set-up"}.</>} right={<div className="flex flex-wrap items-center gap-2"><Link href={`/squad/${slug}`} className="focus-ring inline-flex items-center rounded-lg bg-mint px-4 py-2.5 text-sm font-semibold text-night hover:bg-mint-soft">Your player page →</Link><SignOut /></div>} />
      {!sheetsConfigured() && <p className="card border-gold/40 p-4 text-sm text-ash">Saving is not switched on yet: the site cannot write to the records sheet until the Google service account is set up (see LAUNCH.md). You can look, but the save button will tell you the same.</p>}
      <ProfileForm initial={initial} />
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        {([["Submit a score", "/submit", "Recorded as soon as you send it."], ["Log a payment", `/submit?type=payment&player=${encodeURIComponent(member.player)}`, "Goes straight onto the Payments tab."], ["Add a player", "/submit?type=player", "New signings join the roster instantly."]] as const).map(([t, href, sub]) => (
