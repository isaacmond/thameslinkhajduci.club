    ...
    <>
      ...
      <ol className="space-y-4">
        {seasons.map((s) => {
          ...
          return (
            <li key={s.id}>
              <Link href={`/seasons/${s.id.toLowerCase()}`} className="focus-ring card group grid gap-5 p-5 transition-colors hover:border-white/20 lg:grid-cols-[auto_1fr_auto_auto] lg:items-center">
                <div className="flex items-center gap-4 lg:w-44">
                  ...
                  <div><h2 className="display text-2xl leading-none text-cream">Season {s.number}</h2><p className="text-xs text-ash">{s.period}</p>{s.isCurrent && <Tag tone="mint" className="mt-1">In progress</Tag>}</div>
                </div>
                <div>
                  ...
                  <div className="mt-2"><FormStrip matches={form(s.matches, 8)} size="sm" /></div>
                </div>
