            ...
            <li key={s.id}>
              <article className="card group relative grid gap-5 p-5 transition-colors hover:border-white/20 lg:grid-cols-[auto_1fr_auto_auto] lg:items-center">
                <div className="flex items-center gap-4 lg:w-52">
                  <span className={clsx("display flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl text-3xl leading-none [&>*]:translate-y-[0.05em]", s.isCurrent ? "bg-mint text-night" : "bg-white/[0.06] text-cream")}>{s.id}</span>
                ...
                <dl className="grid grid-cols-4 gap-x-2 text-center sm:grid-cols-7 lg:w-[23rem]">
                  {([["P", s.summary.played, "text-cream"], ["W", s.summary.won, "text-mint-soft"], ["D", s.summary.drawn, "text-draw-soft"], ["L", s.summary.lost, "text-loss-soft"], ["GF", s.summary.goalsFor, "text-cream"], ["GA", s.summary.goalsAgainst, "text-cream"], ["GD", signed(gd), gd >= 0 ? "text-mint-soft" : "text-loss-soft"]] as [string, number | string, string][]).map(([k, v, c]) => <div key={k}><dd className={clsx("display tabular text-2xl", c)}>{v}</dd><dt className="text-[10px] uppercase tracking-wider text-ash">{k}</dt></div>)}
                </dl>
      ...
      {data.friendlies && (
        <article className="card mt-6 flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4">
            <span className="display flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-white/[0.06] text-2xl leading-none text-cream"><span className="translate-y-[0.05em]">FR</span></span>
            <div>
