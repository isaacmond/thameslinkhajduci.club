...
export default async function SubmitPage({ searchParams }: { searchParams: Params }) {
  ...
  if (kind === "payment") {
    ...
    form = <PaymentForm players={players} payer={(current && data.money.paidBy[current.id]) || null} initialPlayer={player} today={today} />;
  } else if (kind === "player") {
    ...
    form = <PlayerForm roster={roster} takenShirts={takenShirts} seasonId={current?.id ?? null} />;
  } else {
    ...
    form = <ScoreForm fixtures={[...past, ...future].map(toFixture)} roster={roster} initialMatch={match} webhook={Boolean(process.env.SCORE_WEBHOOK_URL || (process.env.RESEND_API_KEY && process.env.SCORE_TO_EMAIL))} />;
  }
