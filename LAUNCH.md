...
- **Man of the match vote**: the moment a league result is recorded (a signed-in member's submission, or your approval of a queued one), everyone recorded as having played who has an address on the members list gets a ballot by email from `matches@thameslinkhajduci.club`: the team-mates as tappable names (one tap opens `/motm/<token>` with that name picked, one more confirms; you cannot vote for yourself, and you can change your mind). The poll closes after **48 hours or when every ballot is in**, whichever is first, and the winner is written to the fixture's MOTM (over whatever the score form said; ties go to goals in the game, then assists, then who got there first). Everyone with a ballot gets the result by email. Friendlies and forfeits never get one; players without an email can be picked but cannot vote. A corrected line-up moves the ballots with it; a forfeit correction cancels the vote. Tables `motm_polls` and `motm_ballots` (migration 0003). Closing on time: the last ballot in, a ballot opened after the deadline, or the daily cron (`/api/cron/reminders` sweeps due polls on both its runs; Hobby crons cannot run hourly, so a poll nobody touches is declared at the next 17:00/18:00 UTC run). `/admin#motm` shows open and decided votes, lets you close one early, re-send missing ballots, or open one on an older result.
