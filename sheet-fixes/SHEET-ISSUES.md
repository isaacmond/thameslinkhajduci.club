...
## 2.11 Aliases and Opponents tabs (new in the corrected copy)
Two small lookup tabs let you fix names without touching hundreds of cells or asking for a code change:
- **Aliases** (`From`, `To`): any player name written as *From* anywhere in the season tabs (roster rows, MOTM cells, Money, Payments) is treated as *To*. Pre-filled with Robin → Robin Watson, Seb → Seb Burgess, Eddie Ringer → Eddie McLaughlin and the old app's Jake Hamilton/Jacob Hamilton → Jake Ringer.
- **Opponents** (`Spelling`, `Canonical`): any opponent written as *Spelling* (case and punctuation ignored) is shown as *Canonical*. Pre-filled with every variant found in the audit.
The site re-reads both tabs on every refresh, so a new spelling is one row away from being fixed. (The 29 opponent-name cell edits in section 1 are still applied so the sheet itself reads cleanly; with the Opponents tab in place they are no longer required for the site.)
