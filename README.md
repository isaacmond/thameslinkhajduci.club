...
- **Rules:** friendlies and forfeits (the `Type` row, or anything in a `Friendlies` tab) are shown but excluded from W/D/L, goals and player totals. Whoever is named in a season's "Paid by" cell is credited the pitch cost of played games. A forfeit's line-up is who pays for the pitch (the people responsible, picked on the score form), not who played; leave it empty and nobody is charged. Goals-per-game only counts games where scorers were logged; assists-per-game only counts games where assists were logged. Opponent names are normalised so head-to-head records line up.
